\chapter{Que podria fer en un futur?}
...
%Fa més de dos anys que vaig començar a aprendre conceptes de computació quàntica, ha sigut un camí molt llarg, però que m'ha encant de recórrer. Pensar en tot el que he après em dona una gran alegria. Els camps dels quals he parlat realment m'encanten (computació quàntica i intel·ligència artificial). A més a més, aprendre tot això m'ajudaria molt en cas que estudiï una carrera en física, que és el meu objectiu.
...
Finalment, he pensat en veure com codificar imatges en color en un circuit quàntic. En una primera instancia pots pensar que s'hauria de fer mitjançant una codificació en RGB. Una tasca que ja ha sigut realitzada \cite{qrgb}. No obstant això, he tingut l'idea d'utilitzar una codificació en YCbCr. En comptes de fer servir els vermell, blau i verd; es faria servia un valor de lluminositat, un to de color entre el blau i el groc, i un altre entre el vermell i el verd. Bàsicament és seguir utilitzant tres valors per píxel però els qual representen diferents colors que no són RGB. 

Codificar les imatges en YCbCr dona la possibilitat de fer \textit{subsampling}, una tècnica de compressió d'imatges utilitzada en formats com JPEG \cite{wiki:sub}.  Per no dedicar molt de temps a aquesta qüestió, només mencionaré que realitzar \textit{subsampling}, permetria per exemple que cada dos píxels comparteixin els mateixos valor pels color, però que cadascun tingui un valor de lluminositat propi. D'aquesta manera es requeriran emmagatzemar menys nombres per píxel.
...
Per una funció d'ona $\Psi(x,t)$ que representa una partícula, la probabilitat de trobar aquesta partícula en un punt $x$ és $\abs{\Psi(x,t)}^2$. Per tant, la funció d'ona ha de ser normalitzada seguint la fórmula:
\begin{equation}
...
$$
Ara amb l'equació de Schrödinger:
$$
\pdv{\Psi}{t} = \frac{i\hslash}{2m}\pdv[2]{\Psi}{x} -  \frac{i}{\hslash}V\Psi
$$
A continuació calculem el conjugat complex de la derivada anterior:
$$
\pdv{\Psi^*}{t} = - \frac{i\hslash}{2m}\pdv[2]{\Psi^*}{x} + \frac{i}{\hslash}V\Psi^*
$$
Quan s'ha d'arregla d'aquesta forma:
$$
...
$$
Ja que $\Psi(x,t)$ ha de convergir a zero quan $x$ va cap a infinit, es veritat que:
$$
...
Aquesta demostració amb un vector d'estat és molt més simple: al evolucionar a partir de matrius unitàries, les quals preserven la norma del vector a la qual s'apliquen; si el vector al qual s'aplica una porta quàntica té una norma de $1$, aquesta norma es preservarà.
...
La \textit{Big-O Notation} consisteix en definir el nombre màxim d'operacions que necessita un algoritme, es denota com $O(\cdot)$ on l'argument usualment depèn de $n$ que és la mida de l'input a l'algoritme, per exemple un algoritme de cerca ha de cercar a través de $n$ coses. 

Com a exemple: un algoritme de cerca de cadenes binàries corre en un temps $O(\log_{2} n)$, on $n$ és el nombre de cadenes entre les quals ha de cercar. La notació $O(\cdot)$ es diu que és el \textit{upper boundary}, això significa que $\log_{2} n$ és la quantitat de temps més gran en la qual es trobaria la cadena. Això vol dir que és possible que a la primera comprovació aconsegueixi trobar la cadena que es cerca per exemple.

Simplement, aquesta notació és una manera d'avaluar com d'eficients són els algoritmes amb relació a la mida de l'input que tenen. Cal notar que les operacions específiques que es quantifiquen varien entre els tipus d'algoritmes. Cada grup d'algoritmes que tenen la mateixa tasca, tenen una operació a quantificar diferent.
...
En 1996, Lov Grover va presentar un algoritme quàntic per cercar entre una sèrie de dades desordenades \cite{Grover_96} (e.g. cercar el número de telèfon en una llista desordenada). Per aquest problema un algoritme clàssic té una complexitat de $O(N)$ cerques\footnote{Una cerca completada és quan es verifica si un element de la llista és l'element que se cerca o quan no es troba aquest nombre al comprovar tots els elements de la llista }, mentre que l'algoritme de Grover té una complexitat de $O(\sqrt{N})$, sent substancialment més eficient. En les paraules de Grover \cite{Grover_96} (adaptades): un ordinador clàssic per tindre una probabilitat de $\frac{1}{2}$ de trobar un nombre de telèfon d'una persona en una llista desordenada necessita mirar a un mínim de $\frac{N}{2}$ números, mentre que amb el seu algoritme s'obté el número de telèfon en només $O(\sqrt{N})$ passos\footnote{Per passos entenc que es refereix al nombre de vegades que es mira a l'oracle, és a dir, el nombre que de vegades que es verifica si s'ha trobat l'element que se cerca.}.

L'algoritme funciona de la següent manera:

\textbf{1.} S'agafa un sistema de $n$ qubits en l'estat $\ket{0}$ que resulten en una combinació de $N = 2^n$ estats. S'aplica una distribució uniforme als qubits, és a dir, aplicar portes Hadamard a tots els qubits, tenint al final un estat $\ket{s}$:
\begin{equation*}
...
\begin{algorithmic}[1]
	\State{Aplicar l'\textit{Oracle} $U_{\omega}$}
...
No entraré més en profunditat sobre aquest algoritme, perquè si no hauré d'introduir més conceptes de computació quàntica com ara la fase d'un estat, o començar a utilitzar notació més complicada que també hauré d'explicar. Em sap greu, perquè és un algoritme que funciona d'una manera molt maca de veure.

També em sembla un bon algoritme per poder explicar una de les millors característiques de la computació quàntica: és possible descartar automàticament els estats dolents dintre d'una combinació d'estats, d'aquesta manera lliurant un estat concret desitjat d'entre la combinació. En el cas de l'algoritme de Grover aquest estat desitjat és l'estat que està cercant l'\textit{oracle}.

\section{Algoritme de Shor}
No explicaré aquest algoritme en profunditat perquè és troba fora de l'àmbit d'aquest treball\footnote{Hauria d'introduir nous conceptes com la fase d'un qubit i com efectuar la transformada de Fourier inversa en un circuit quàntic, a més a més hauria d'explicar-lo en una notació bastant complexa. Però, es pot entendre com funciona sense cap problema veient l'entrada de Wikipedia sobre aquest algoritme.}. Em limitaré a explicar quina és la seva utilitat i el compararé amb les seves alternatives clàssiques.

L'algoritme de Shor per la factorització en nombres primers \cite{Shor_97}, és probablement l'algoritme quàntic més famós i més rellevant que hi ha. Això és a causa de la gran diferencia en complexitat que té en comparació als seus anàlegs clàssics, i a causa del gran impacte que tindria la seva implementació a gran escala. Aquest últim punt és el més interessant i cridaner, per tant, és el que desenvoluparé en major profunditat.

Actualment, totes les dades encriptades que s'envien a través d'internet són encriptades mitjançant la família de protocols RSA\footnote{RSA són les sigles dels creadors d'aquests protocols; Ron Rivest, Adi Shamir i Leonard Adleman, que en 1977 van publicar els resultats de la seva investigació \cite{RSA}.}. Aquests protocols són una mica complicats, no els explicaré. L'únic fet que hem de tenir en compte és que per desencriptar un missatge s'ha de trobar els factors prims d'un nombre $n$, és a dir, dos nombres primers $p$ i $q$ que multiplicats donin $n$. Amb aquests nombres es poden saber les claus públiques i privades per encriptar i desencriptar missatges. $n$ seria la clau pública, un nombre que tothom pot conèixer i amb el qual es poden encriptar missatges. Mentre que a partir de $p$ i $q$ es pot trobar la clau privada que correspon a la clau pública. Amb aquesta clau es pot desencriptar un missatge encriptat amb $n$, per aquesta raó es mantenen els nombres $p$ i $q$ secrets. Es pot entendre la clau pública com un cadenat que només s'obre amb la clau privada. Cal mencionar que a cada clau privada correspon una clau pública, són paralles úniques.
...
Això és a causa de la petita quantitat de qubits dels quals disposen els ordinadors quàntics actuals i als errors que aquests donen a terme a l'hora d'aplicar operacions. Quan un ordinador quàntic aplica una rotació en l'estat d'un qubit físic, aquest ho fa amb una certa imprecisió. El mateix passa a l'hora d'aplicar portes a dos qubits, com per exemple una $\mathrm{CNOT}$. Els ordinadors quàntics actuals només són utilitzats per fer experiments, com provar nous algoritmes i valorar els mateixos ordinadors. 

Són extremadament poques les situacions pràctiques en les quals els ordinadors quàntics actuals siguin més útils que els clàssics. Tanmateix, és una tecnologia molt prometedora, en la qual grans companyies com Google, IBM, Amazon estan invertint per investigar-la, igual que altres companyies més petites i nombrosos equips d'investigació al llarg del món. S'espera que un futur els ordinadors quàntics ens permetran dur a terme tasques que són impossibles de realitzar en ordinadors clàssics, sobretot en tenir en compte que s'espera que començarem a arribar a limitacions físiques si continuem millorant els processadors d'ordinadors actuals.

No obstant això, aquesta afirmació s'ha de agafar amb cautela. És una tecnologia que s'està desenvolupant actualment, no és pot afirmar amb total seguretat que arribarà a un nivell en concret. És completament possible que al cap d'uns anys els investigadors es trobin amb una limitació física que és impossible de solucionar i que els ordinadors quàntics quedin rellevants com una tecnologia que va prometre molt i no va deliberar. L'única cosa que es pot fer és seguir investigant.

Però deixant tot això de banda, dels continguts d'aquest annex, el més rellevant són els algoritmes. Tant el seu funcionament, com la comparació que vaig entre els quàntics i els clàssics. Valorar el nivell tecnològic actual del ordinadors quàntics no és el focus del treball, i per tant, no tinc molta confiança en les afirmacions sobre aquest tema. Tanmateix, les he inclòs perquè penso que és una qüestió que s'ha d'adreçar. Encara que no he fer una recerca que fa justícia a aquesta qüestió. 


\chapter{Codi}
En l'annex actual presentaré el codi que he utilitzat al llarg del treball. Està organitzat segons el moment en el qual he referenciat el codi en el text. Lamento no poder explicar el codi profundament, ja que no era pertinent. M'hauria agradat fer-ho. 

He de mencionar que és possible que hi hagin errors en els espais o la indexació del codi presentant en aquestes pàgines del treball. Per una revisió exhaustiva del codi,  consulteu \href{https://github.com/tomiock/quantum-GAN}{el repositori de \textit{Github}}.

Aquest projecte de \href{www.python.org}{\textit{Python}} el vaig començar a desenvolupar mitjançant una \href{https://jupyter.org/}{\textit{Jupyter Notebook}}, a través de \href{https://colab.research.google.com/}{\textit{Google Colab}}. Però quan el projecte va augmentar de mida i necessitava més eines (com un \textit{debugger}\footnote{Un \textit{debugger} és una eina que serveix per diagnosticar i arreglar problemes en el codi, és extremadament útil. }), vaig decidir passar-me a un IDE (\textit{Integrated Development Environment}), un programa que pose més recursos en les mans del programador. Vaig decidir-me per un IDE expressament dissenyat per a \textit{Python}: \href{https://www.jetbrains.com/pycharm/}{\textit{PyCharm}}. Concretament per \textit{PyCharm Community}, que és d'ús gratuït. Per poder manejar els paquets de \textit{Python} he utilitzat \href{https://www.anaconda.com/products/individual}{\textit{Anaconda}}. Aquestes eines o programes en van facilitar molt la feina. Aquest \textit{workflow} potser és una mica avançat, però he arribat a un punt en el qual sé com funciona tot i no penso canviar-lo en un futur tret que hagin millors opcions\footnote{Com per exemple \href{https://www.jetbrains.com/fleet/}{\textit{JetBrain Fleet}}, però ho dubto, que \textit{Pycharm} m'encanta.}. Finalment he de mencionar que tots els programes o recursos que he utilitzat en aquest treball són gratuïts/accessibles.

Els paquets que són necessaris per executar el codi estan especificats en el \href{https://github.com/tomiock/quantum-GAN/network/dependencies}{repositori del treball}. Tota la informació corresponent a aquest treball es pot trobar en el \href{https://github.com/tomiock/quantum-GAN}{meu repositori de \textit{Github}}. Des dels arxius de \LaTeX \ o \textit{Python}, fins als canvis que vaig anar fent al llarg del treball. 
