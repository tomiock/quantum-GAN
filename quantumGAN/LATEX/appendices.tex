...
Actualment totes les dades encriptades que s'envien a través d'internet són encriptades mitjançant la familia de protocols RSA\footnote{RSA són les sigles dels creadors d'aquests protocols; Ron Rivest, Adi Shamir i Leonard Adleman, que al 1977 van publicar els resultats de la seva investigació.  https://dl.acm.org/doi/10.1145/359340.359342}. Aquests protocols es basen en el teorema petit de Fermat, el qual afirma que per un nombre enter $a$ i per un nombre prim $p$, el resultat de $a^p - a$, ha de ser un múltiple de $p$, en altres paraules, $a^p = a \pmod p$. Utilitzant aquest teorema es poden trobar facilment múltiples de nombres, llavors si 
