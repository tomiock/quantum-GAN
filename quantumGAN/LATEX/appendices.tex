...
\begin{enumerate}
	...
	\end{align*}
	On el signe $\pm$ indica la diferència entre la diferència entre la polarització circular cap a la dreta o l'esquerra, amb $+$ i $-$, respectivament.
...
\label{complexity}
En ciència de la computació existeix el concepte de \textit{Big-O Notation}, una forma de quantificar l'eficiència dels algoritmes per fer certes tasques, en altres paraules la complexitat dels algoritmes. Bàsicament, és una forma de classificar-los segon la rapidesa que tenen en fer la tasca que els correspon, aquesta rapidesa no és mesura en segons, ja que aquesta mètrica pot variar d'ordinador a ordinador per les diferències en hardware que aquest poden tindre. En canvi, es mesura segons nombre d'operacions o temps directament, però sense unitats.
